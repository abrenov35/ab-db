(function(){
  ...
  async function loadClientFolder(folder){clientFolderBtns.forEach(function(b){b.classList.toggle('active',b.dataset.folder===folder);});clientsStatus.textContent='Chargement '+folder+'…';clientsResults.innerHTML='';try{const data=await jsonp({action:'clientFolders',folder:folder});if(!data||!data.ok)throw new Error(data&&data.error?data.error:'Erreur inconnue');const items=data.items||[];renderInto(clientsResults,items,'Aucun dossier dans '+folder+'.');clientsStatus.textContent=items.length+' dossier(s).';}catch(err){clientsResults.innerHTML='<div class="empty">Impossible de charger les dossiers.</div>';clientsStatus.textContent='Erreur : '+err.message;}}
  ...
  function toggleClients(){if(!clientsView.hidden){showOnly(dropboxView);resetButtons();return;}showOnly(clientsView);input.hidden=true;clientsBtn.textContent='← Retour recherche';abOppBtn.textContent='📋 Opportunités';opportunitiesBtn.textContent='📁 Opportunités à traiter';}
  async function toggleOpportunities(){if(!opportunitiesView.hidden){showOnly(dropboxView);resetButtons();return;}showOnly(opportunitiesView);input.hidden=true;clientsBtn.textContent='👥 Clients AB RENOV 35';abOppBtn.textContent='📋 Opportunités';opportunitiesBtn.textContent='← Retour recherche';await loadOpportunities();}
  async function toggleAbOpp(){if(!abOppView.hidden){showOnly(dropboxView);resetButtons();return;}showOnly(abOppView);input.hidden=true;clientsBtn.textContent='👥 Clients AB RENOV 35';opportunitiesBtn.textContent='📁 Opportunités à traiter';abOppBtn.textContent='← Retour recherche';await loadAbOpp();}
