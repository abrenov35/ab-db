(function(){
  ...
  function resetButtons(){setClientsMode(false);clientsBtn.textContent='👥 Clients AB RENOV 35';abOppBtn.textContent='+ Opportunité';opportunitiesBtn.textContent='📁 Opportunités à traiter';input.hidden=false;clientFolderBtns.forEach(function(b){b.classList.remove('active');});}
  ...
  function toggleClients(){if(!clientsView.hidden){showOnly(dropboxView);resetButtons();return;}showOnly(clientsView);setClientsMode(true);input.hidden=false;clientsBtn.textContent='👥 Clients AB RENOV 35';abOppBtn.textContent='+ Opportunité';opportunitiesBtn.textContent='📁 Opportunités à traiter';}
  async function toggleOpportunities(){if(!opportunitiesView.hidden){showOnly(dropboxView);resetButtons();return;}showOnly(opportunitiesView);setClientsMode(false);input.hidden=false;clientsBtn.textContent='👥 Clients AB RENOV 35';abOppBtn.textContent='+ Opportunité';opportunitiesBtn.textContent='← Retour recherche';await loadOpportunities();}
